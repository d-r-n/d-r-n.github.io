...
comments: true
title: "ZeroVote"
excerpt: "I built a voting system that guarantees fair elections."
date: 2026-04-12
mathjax: false
---
...
**How do you reject an invalid ballot without reading it?** Every ballot includes a zero-knowledge proof — [Bulletproofs R1CS](https://eprint.iacr.org/2017/1066.pdf) with [Pedersen commitments](https://www.cs.cornell.edu/courses/cs754/2001fa/129.PDF) on Curve25519 — that proves the vote is well-formed without revealing what it contains. The server can verify that a ballot selects exactly one candidate (or provides a valid ranking) and reject anything malformed, without ever learning what the vote says. The Pedersen commitment blinding factors — the values that could theoretically reverse-engineer the vote — never leave the voter's device.

**How do you know the server didn't tamper with the ballots it stored?** Every accepted ballot goes into a [BLAKE3](https://github.com/BLAKE3-team/BLAKE3) Merkle tree, and every voter gets an inclusion proof as a receipt. If the server adds, removes, or modifies any ballot after the fact, the root hash changes and every receipt holder can detect it.
